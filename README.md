...
> ### which is all my notes and changes which i changed or replaced in the ndarray
...
> All [LICENSE](https://github.com/GlitchHunterCoder/Package-NextNdarray/blob/main/LICENSE) notices have been adhered to, all copyright notices have been preserved
...
# Changes
- ## Evaled Function to Modern JS
throughout, this tool used a structure inside `function compileConstructor` which utilised a lot of `code.push(...)` in order to dynamically build code to execute

note: the original eval pattern had genuine performance benefits
for tight loops — this rewrite prioritises readability and 
maintainability over that micro-optimisation

This was a good idea for a long while, it served many purposes:
- faster engine optimisations,
- expanding capabilities of JS,
- and control over how code executes based on outside variables

and the solution was novel, for its time, in 2013, but now at time of refactor, it is 2026 now, and so the solution used here is outdated now

---

when ndarray was written, it was 2013

  - `ndarray`: 2013 -> Latest: ECMAScript 5 (ES5), Uses: mostly ECMAScript 3 (ES3)
    - `Additions for Latest`
    - JavaScript "strict mode"
    - JS getters and setters
  
  since then, we had all the way to 2026
  
  - `NextNdarray`: 2026 -> Latest: ECMAScript 2023 (ES14), Uses: mostly ECMAScript 6 (ES6)
    - `Additions for Latest`
    - toSorted method
    - toReversed method
...
since then the following features which ive used have arrived

- `Differences` 2013-2026 -> ES5-ES14
  - let and const `ES6`
  - Arrow functions `ES6`
  - Block scope for variable `ES6`
  - spread operator `ES6` `[KEY PART]`
  - rest parameters `ES6` `[KEY PART]`
  - Computed keys `ES6` `[KEY PART]`
  - Optional chaining `ES2020`
  - Array.toSorted `ES14`
  - and a lot more

so with ES6, the combination of these key features made the eval/code.push pattern redundant

it just took until 2026 for someone to actually rewrite it
